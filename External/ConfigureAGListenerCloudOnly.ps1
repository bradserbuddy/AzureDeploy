...
ForEach ($node in $WSFCNodes)
{
    ...
    $out = Invoke-Command -ConnectionUri $uri.ToString() -Credential $credential -ArgumentList $AGName, $WSFCNodes, $ListenerName, $firstIteration -ScriptBlock `
    {
        ...
        If ($FirstIteration)
        {
            ...
            If ((Get-ClusterNetwork).Count -ne 1) 
            {
...
ForEach ($node in $AGNodes)
{
    ...
    $credential = New-Object System.Management.Automation.PSCredential("$DomainAccount", $(ConvertTo-SecureString $Password -AsPlainText -Force))
    $uri = Get-AzureWinRMUri -ServiceName $ServiceName -Name $node
    Invoke-Command -ConnectionUri $uri.ToString() -Credential $credential -ArgumentList $AGName, $ListenerName, $serviceip, $EndpointPort -ErrorAction Stop -ScriptBlock `
    {
        param($AGName, $ListenerName, $IPAddress, $Port)
        ...
        netsh advfirewall firewall add rule name='Availability Group Listener (TCP-In)' localport=$Port dir=in action=allow protocol=TCP | Out-Null
        ...
        If ((Get-ClusterGroup $AGName).OwnerNode -eq $env:COMPUTERNAME)
        {
            ...
            Set-SqlAvailabilityGroupListener -Path SQLSERVER:\SQL\$env:COMPUTERNAME\DEFAULT\AvailabilityGroups\$AGName\AvailabilityGroupListeners\$ListenerName -Port $Port -ErrorAction Stop | Out-Null
        }
    }
...
Write-Host "sqlcmd -S $ListenerName,$EndpointPort -d <DATABASENAME> -Q 'select @@servername, db_name()' -l 15"
