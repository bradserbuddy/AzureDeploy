...
Invoke-Command -Session $session -FilePath $workingDir"Sql\InstallAvailabilityGroupPrep1.ps1" -ArgumentList $sql1ServerName, $sqlUserName1, $vmAdminPassword
Remove-PSSession -Session $session
$session = GetSession "$domainNameAsPrefix$installUserName" $vmAdminPassword $sqlCloudServiceName $sql2ServerName
Invoke-Command -Session $session -FilePath $workingDir"Sql\InstallAvailabilityGroupPrep2.ps1" -ArgumentList $sql2ServerName, $sqlUserName2, $vmAdminPassword
Remove-PSSession -Session $session
